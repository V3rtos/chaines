...
public final class ArgumentWrapperImpl implements ArgumentWrapper {

    @Getter
    private String parentName;
    private String[] handle;

    private ArgumentWrapperImpl(String[] handle) {
        this.handle = handle;
    }

    private ArgumentWrapperImpl(String parentName, String[] handle) {
        this.parentName = parentName;
        this.handle = handle;
    }

    private ArgumentWrapperImpl() {}

    public static ArgumentWrapperImpl withLine(String line) {
        String[] array = line.split(" ");
        String[] formattedArray = Arrays.copyOfRange(array, 1, array.length);

        String parentName = array[0];

        return new ArgumentWrapperImpl(parentName, formattedArray);
    }
